...
class Gen_Model():
	...
	def read(self, game, version):
		return load_model( run_archive_folder + "models/version" + "{0:0>4}".format(version) + '.h5', custom_objects={'sparse_categorical_crossentropy':tf.keras.losses.SparseCategoricalCrossentropy()})
...
class Residual_CNN(Gen_Model):
	...
	def _build_model(self):
		...
		model.compile(loss={'value_head': 'mean_squared_error', 'policy_head': softmax_cross_entropy_with_logits},
			optimizer=SGD(lr=self.learning_rate, momentum = config.MOMENTUM, clipnorm=1.0),	
