...
def playMatchesBetweenVersions(env, player1version, player2version, EPISODES, turns_until_tau0, goes_first = 0):
    
    if player1version == -1:
        player1 = User('player1', env.state_size, env.action_size, env.grid_shape)
    else:
    ...
    if player2version == -1:
        player2 = User('player2', env.state_size, env.action_size, env.grid_shape)
    else:
    ...
    scores, memory, points, sp_scores = playMatches(player1, player2, EPISODES, turns_until_tau0, goes_first)

    return (scores, points, sp_scores)


def playMatches(player1, player2, EPISODES, turns_until_tau0, goes_first = 0, memory = None):
